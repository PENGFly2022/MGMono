...
class SelfAttention(nn.Module):
    def __init__(self, in_channels, options):
        super(SelfAttention, self).__init__()  # 调用方式
        self.opt = options
    ...
    def forward(self, x):
        ...
        query = self.query_conv(x.to(self.device)).view(batch_size, -1, height * width)
        key = self.key_conv(x.to(self.device)).view(batch_size, -1, height * width).permute(0, 2, 1)
        value = self.value_conv(x.to(self.device)).view(batch_size, -1, height * width)

        attention = torch.matmul(query, key)  # 计算注意力权重
        attention = torch.softmax(attention, dim=-1)  # softmax得到权重
...
class Trainer:
    def __init__(self, options):
        ...
        self.models["encoder"] = networks.MGMono(model=self.opt.model,options=options,
    ...
    def process_batch(self, inputs):
        ...
        if self.opt.predictive_mask:
            # print("---------------self.opt.predictive_mask-------------")
            outputs["predictive_mask"] = self.models["predictive_mask"](features)
        if self.use_pose_net:
            # print("---------------self.use_pose_net-------------")
    ...
    def predict_poses(self, inputs, features):
        ...
        if self.num_pose_frames == 2:
            ...
            if self.opt.pose_model_type == "shared":
                # print("---------------self.opt.pose_model_type == shared-------------")
            ...
            for f_i in self.opt.frame_ids[1:]:
                if f_i != "s":
                    ...
                    if f_i < 0:
                        pose_inputs = [pose_feats[f_i], pose_feats[0]] # -1帧input_aug与当前帧input_aug
                    else:
                        pose_inputs = [pose_feats[0], pose_feats[f_i]]

                    if self.opt.pose_model_type == "separate_resnet":
                        # print("---------------self.opt.pose_model_type == separate_resnet-------------")
    ...
    def val(self):
        ...
        with torch.no_grad():
            outputs, losses = self.process_batch(inputs) # 处理验证数据并计算损失
    ...
    def pixelwise_cosine_similarity_loss(self, input1, input2, epsilon=1e-8):
        """
        逐像素计算两个特征图之间的余弦相似度损失

        参数:
        input1: 第一个特征图张量，形状为 (batch_size, channels, height, width)
        input2: 第二个特征图张量，形状为 (batch_size, channels, height, width)
        epsilon: 防止除以零的小常数
        ...
        # 计算余弦相似度
        cos_sim = dot_product / (norm_input1 * norm_input2 + epsilon)

        # 将余弦相似度值限制在[-1, 1]范围内
        cos_sim = torch.clamp(cos_sim, -1, 1)

        # 计算余弦相似度损失
        loss = self.weight *(1 - cos_sim)
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            if self.opt.avg_reprojection:
                ...
                # print("reprojection_loss-----------",reprojection_loss.shape)
            else:
        ...
        losses['loss_a'] = total_loss
        # print(',',total_loss)
        cos_losses /= self.num_scales
        losses['loss_cos'] = cos_losses
        a = 0.1
        # print(total_loss)
        # print(total_loss.shape,"------total_shape------")
        losses["loss"] = (1-a)*total_loss + a*losses['loss_cos']
    ...
    def cosine_similarity_loss(self, feature_map1, feature_map2, epsilon=1e-8):
        """
        计算两张特征图之间的余弦相似度损失。

        参数:
        feature_map1: 第一张特征图，形状为 (batch_size, num_channels, height, width)
        feature_map2: 第二张特征图，形状为 (batch_size, num_channels, height, width)
        epsilon: 为了数值稳定性添加的小常数

        返回:
        余弦相似度损失
        """
        # 将特征图的形状从 (batch_size, num_channels, height, width) 改变为 (batch_size, num_channels*height*width)
        feature_map1 = feature_map1.view(feature_map1.size(0), -1)
        feature_map2 = feature_map2.view(feature_map2.size(0), -1)

        # 计算feature_map1和feature_map2的余弦相似度
        cos_sim = F.cosine_similarity(feature_map1, feature_map2, dim=1, eps=epsilon)

        # 将余弦相似度转换为损失，通常取负值，因为目标是最大化相似度
        # loss = 1.-cos_sim.mean()
        if 1-cos_sim.mean() < 0.15:
            # loss = 0.000000001
            loss = (1-cos_sim.mean())/1000000
        else:
            loss = 1-cos_sim.mean()
    ...
    def log_time(self, batch_idx, duration, loss, loss_c, loss_a):
        ...
        print_string = "epoch {:>3} | lr {:.6f} |lr_p {:.6f} | batch {:>6} | examples/s: {:5.1f}" + \
            " | loss: {:.5f} | loss_cos: {:.5f} |loss_a: {:.5f} || time elapsed: {} | time left: {}"
        print(print_string.format(self.epoch, self.model_optimizer.state_dict()['param_groups'][0]['lr'],
                                  ...
                                  batch_idx, samples_per_sec, loss,loss_c,loss_a,
