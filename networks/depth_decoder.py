...
class DepthDecoder(nn.Module):
    def __init__(self, num_ch_enc, scales=range(3), num_output_channels=1, use_skips=True):
        ...
        # self.upsample_mode = DySample(num_ch_enc)
        # self.upsample_mode = 'bilinear'
        # self.dysam = DySample(num_ch_enc[1])
        self.scales = scales
        ...
        for i in range(2, -1, -1):
            # upconv_0
            dysam_block.append(DySample(self.num_ch_dec[i]))
            num_ch_in = self.num_ch_enc[-1] if i == 2 else self.num_ch_dec[i + 1]
            num_ch_out = self.num_ch_dec[i]
            self.convs[("upconv", i, 0)] = ConvBlock(num_ch_in, num_ch_out)
            # print(i, num_ch_in, num_ch_out,'ggggggggggggggggggg')
        ...
        self.dysam = nn.ModuleList(dysam_block)
    ...
    def forward(self, input_features):
        ...
        for i in range(2, -1, -1):
            ...
            if i in self.scales:
                # print(self.convs[("dispconv", i)](x).shape,'self.convs[("dispconv", i)](x)')
                f = upsample(self.convs[("dispconv", i)](x), mode='bilinear')
                # f = self.dysam1(self.convs[("dispconv", i)](x))
